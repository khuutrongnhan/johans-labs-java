...
public class FIFO implements Queue {
	...
	private int maxSize = 0; // the maximum size this queue has had since it was created.
	
	
	
	/** Add item to queue
	 * @param arg0 The item to add to the queue
	 */
	 ...
	 * @throws NoSuchElementException The queue must not be empty (if it is, a java.util.NoSuchElementException is thrown).
	 ...
	 * @return boolean returns true if the size of this Queue is 0, otherwise it returns false.
	 */
	 ...
	 * @param f The other queue to compare with
	 * @return boolean True if equal
	 */
	public boolean equals(Object f){
		// pre: f must be of the same type as this class. Otherwise, a ClassCastException is thrown.
		// post: returns true if and only if all the following holds:
		// * this and f has the same size
		// * For every position in this queue:
		//          o If the position contains a null reference, the corresponding position in f contains a null reference.
		//          o If the position contains a reference to the object o1, the corresponding position in f contains the object o2 such that o1.equals(o2) == true
		...
		if(this.size() == ((FIFO)f).size()){
			for(int i=0;i<this.size();i++){
				if((this.objectVector.elementAt(i) != null) && (((FIFO)f).objectVector.elementAt(i) != null)) {
					if(!(this.objectVector.elementAt(i).equals(((FIFO)f).objectVector.elementAt(i)))) {
						return false;
					}
				}
				else if(this.objectVector.elementAt(i) != ((FIFO)f).objectVector.elementAt(i)){
					return false;
				}
			}
			return true;
